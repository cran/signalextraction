dfa <- function(x, quart = FALSE, d = 0, pb = 1/14, sb = 1/7, tpfilter = FALSE,
                ...
                10, verbose = 1)
  {
    ...
    if (fwing)
      {
        temp <- c(rep(1, n.pg/12+1), (1:(npg2-n.pg/12)))
        ## wgts <- (0:npg2)^2 * exp(fwe*log(0.001+temp))
        lof  <- sum((1:(npg2+1))<=npg2*sb)

        ## altes Baro
        wgts <- c(rep(1,lof),exp(fwe*log(0.001+(0:npg2)))[1:(npg2+1-lof)]) 
      }
    ...
    al <- list(n.pg = n.pg, peclu = peclu, nnull = nnull, pbd = pbd,
               ...
               wgts = wgts, parh = parh, parw = parw, lim = lim, limamp =
               limamp)
        ...
        if (i==1 || any(abs((pobj$value-sdf)/sdf) > 1e-5))
          {
            ...
            pobj  <- optim(parme, funcret, met="Neld", con=co, al=al)
          }
    ...
    for (i in 1:((n.loops+1)/2))
      {
        ...
        co    <- list(REPORT=report, trace=trac, maxit=1000)
        pobj  <- optim(parma, funcret, method="SANN", cont=co, al=al)
        parma <- pobj$par

        info("Nelder-Mead", 2*n.loops+(i-1)*3+2, verbose, n.opt)
        co    <- list(REPORT=report, trace=trac, maxit=1500)
        pobj  <- optim(parma, funcret, method="Neld", cont=co, al=al)
        parma <- pobj$par

        info("BFGS", 2*n.loops+(i-1)*3+3, verbose, n.opt)
        co    <- list(REPORT=report, trace=trac, maxit=100)
        pobj  <- optim(parma, funcret, method="BFGS", cont=co, al=al)
        parma <- pobj$par
      }
